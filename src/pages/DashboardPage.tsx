...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        const authorized = userData?.googleFitAuthorized ?? false;
        setGoogleFitAuthorized(authorized);
      }
      setLoading(false);
    });
  ...
  return (
    <div className='container'>
      ...
      <button style={{ marginBottom: '60px' }} onClick={handleLogout} className='logout-button'>
        Log Out
      </button>
    </div>
  );
